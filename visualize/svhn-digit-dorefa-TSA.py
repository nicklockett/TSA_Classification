...
def get_config():
    ...
    # NOTE: Data block size should be 40 
    ...
    print('training data type: ', type(data_test))"""

    sc = SupervisedClassifier('../../stage1_labels.csv')
    tf_dataset_creator = TensorFlowDataSetCreator(sc)
    dataset = tf_dataset_creator.CreateTensorFlowDataSetFromBlockStream(channels = 3, block_size = 40)
    #data_train = np.array(list(zip(dataset.getTrainingData(),dataset.getTrainingLabels())))
    #data_test = np.array((list(zip(dataset.getTestingData(),dataset.getTestingLabels()))))
    
    data_train = tf.data.Dataset.from_tensor_slices((dataset.getTrainingData(), dataset.getTrainingLabels()))
    data_test = tf.data.Dataset.from_tensor_slices((dataset.getTestingData(), dataset.getTestingLabels()))
